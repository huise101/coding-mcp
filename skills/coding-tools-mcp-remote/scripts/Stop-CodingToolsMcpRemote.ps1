...
$repoDevtunnel = Join-Path $RepoPath "tools\devtunnel.exe"
if (Test-Path -LiteralPath $repoDevtunnel) {
    $resolved = (Resolve-Path -LiteralPath $repoDevtunnel).Path
    Get-Process -Name devtunnel -ErrorAction SilentlyContinue |
        Where-Object { $_.Path -eq $resolved } |
        ForEach-Object { Stop-Process -Id $_.Id -Force -ErrorAction SilentlyContinue }
}
