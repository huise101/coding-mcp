...
When the portable root also contains `skills/windows-remote-server/`, the patched Coding Tools MCP server exposes remote server tools in the same ChatGPT connector:
...
Remote tools use profile names from `<portable-root>/.codex-remote/profiles/*.env` and call `skills/windows-remote-server/scripts/invoke_remote.ps1` or `transfer_remote.ps1`. Do not store SSH passwords, tokens, cookies, private keys, or credential XML in Git. `.codex-remote/` must remain ignored. Password profiles use Windows DPAPI and must be recreated per Windows user/computer; SSH key profiles are the portable option.

Create a profile:

```powershell
cd <portable-root>
.\skills\windows-remote-server\scripts\save_remote_profile.ps1 -Profile "server1" -Root ".\.codex-remote"
```

After the MCP server restarts, use `remote_list_profiles` first, then call remote tools with `profile="server1"`. Local tools still operate only on the selected local workspace; remote tools operate on the server profile's `REMOTE_WORKDIR` or an explicit `remote_dir`.
