...
def _render_server_info(payload: dict[str, Any]) -> str:
    raw_target = payload.get("active_target")
    ...
    if target.get("mode") == "remote":
        ...
        return (
            ...
            f"Active target: REMOTE {profile}:{folder}"
        )
    if target.get("mode") == "unselected":
        return (
            f"{payload.get('server', 'coding-tools-mcp')} {payload.get('version', 'unknown')}\n"
            "Active target: NOT SELECTED"
        )
    workspace = payload.get("workspace") or payload.get("effective_workspace") or "."
    return (
        f"{payload.get('server', 'coding-tools-mcp')} {payload.get('version', 'unknown')}\n"
        f"Active target: LOCAL {workspace}"
    )
...
def _render_exec_environment(payload: dict[str, Any]) -> str:
    ...
    if target.get("mode") == "remote":
        profile = target.get("remote_profile") or ""
        folder = target.get("remote_folder") or "."
        target_text = f"REMOTE {profile}:{folder}"
    elif target.get("mode") == "unselected":
        target_text = "NOT SELECTED"
    else:
        target_text = f"LOCAL {payload.get('workspace') or payload.get('effective_workspace') or '.'}"
    return f"Execution environment checked. Active target: {target_text}. Landlock: {state}.{suffix}"
...
def _render_cwd(payload: dict[str, Any]) -> str:
    ...
    if target.get("mode") == "remote":
        profile = target.get("remote_profile") or ""
        folder = target.get("remote_folder") or "."
        return f"Default working directory: REMOTE {profile}:{folder}"
    if target.get("mode") == "unselected":
        return "Default working directory: NOT SELECTED"
...
def _render_write_file(payload: dict[str, Any]) -> str:
    prefix = "Write validated" if payload.get("dry_run") else "Wrote"
    path = payload.get("path", "")
    bytes_written = payload.get("bytes_written", 0)
    return f"{prefix} {bytes_written} bytes to {path}."
...
def _render_remote_profiles(payload: dict[str, Any]) -> str:
    profiles = payload.get("profiles") if isinstance(payload.get("profiles"), list) else []
    if not profiles:
        return f"No remote profiles found under {payload.get('remote_root', '')}."
    lines = [f"Remote profiles under {payload.get('remote_root', '')}:"]
    for item in profiles:
        if not isinstance(item, dict):
            continue
        host = item.get("host") or "unknown-host"
        user = item.get("user") or "unknown-user"
        port = item.get("port") or "22"
        workdir = item.get("remote_workdir") or "."
        auth = item.get("auth") or "unknown"
        lines.append(f"{item.get('profile', '')}: {user}@{host}:{port} workdir={workdir} auth={auth}")
    return "\n".join(lines)


def _render_remote_read_file(payload: dict[str, Any]) -> str:
    content = payload.get("content")
    if not isinstance(content, str):
        return _render_exec(payload)
    if not payload.get("truncated"):
        return content
    return f"[remote output truncated; raise max_bytes or narrow the range]\n{content}"


def _render_remote_transfer(payload: dict[str, Any]) -> str:
    ...
    elapsed_ms = payload.get("elapsed_ms")
    if isinstance(elapsed_ms, (int, float)):
        parts.append(f"{int(elapsed_ms)} ms")
    lines = [" | ".join(parts)]
    stdout = payload.get("stdout")
    stderr = payload.get("stderr")
    if isinstance(stdout, str) and stdout:
...
_RENDERERS = {
    ...
    "remote_list_profiles": _render_remote_profiles,
    "remote_exec": _render_exec,
    "remote_list_dir": _render_exec,
    "remote_read_file": _render_remote_read_file,
    "remote_list_files": _render_list,
    "remote_search_text": _render_search,
    "remote_write_file": _render_remote_transfer,
    "remote_apply_patch": _render_patch,
    "remote_git_status": _render_git_status,
    "remote_git_diff": _render_git_diff,
    "remote_git_log": _render_git_log,
    "remote_git_show": _render_git_show,
    "remote_git_blame": _render_git_blame,
    "remote_upload": _render_remote_transfer,
    "remote_download": _render_remote_transfer,
