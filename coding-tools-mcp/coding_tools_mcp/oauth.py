...
class OAuthClientRegistry:
    ...
    def register(self, metadata: dict[str, Any]) -> dict[str, Any]:
        ...
        requested_grants = set(grant_types)
        supported_grants = {"authorization_code", "refresh_token"}
        if "authorization_code" not in requested_grants or not requested_grants.issubset(supported_grants):
            raise ValueError("grant_types must include authorization_code and may include refresh_token")
        ...
        with self._lock:
            ...
            client = OAuthClient(
                client_id=client_id,
                redirect_uris=redirects,
                grant_types=tuple(item for item in ("authorization_code", "refresh_token") if item in requested_grants),
                token_endpoint_auth_method=method,
    ...
    def load_jsonable(self, clients: Any) -> None:
        ...
        for item in clients:
            if not isinstance(item, dict):
                continue
            client_id = _optional_text(item.get("client_id"), 512)
            try:
                redirects = validate_redirect_uris(item.get("redirect_uris"))
            except ValueError:
                continue
            grant_types_raw = item.get("grant_types")
            if not isinstance(grant_types_raw, list) or not all(isinstance(grant, str) for grant in grant_types_raw):
                continue
            grant_types = tuple(grant for grant in ("authorization_code", "refresh_token") if grant in set(grant_types_raw))
            ...
            loaded[client_id] = OAuthClient(
                ...
                token_endpoint_auth_method=method,
                client_name=_optional_text(item.get("client_name"), 200),
                secret_digest=_optional_text(item.get("secret_digest"), 256),
                issued_at=issued_at,
            )
        with self._lock:
            self._clients.update(loaded)
...
class OAuthConfig:
    ...
    refresh_tokens_lock: threading.Lock = field(default_factory=threading.Lock)
    state_file: str | None = None


def load_oauth_state(config: OAuthConfig) -> None:
    ...
    if not isinstance(state, dict):
        return
    config.registry.load_jsonable(state.get("clients"))
    refresh_tokens = state.get("refresh_tokens")
    if isinstance(refresh_tokens, dict):
        loaded_tokens: dict[str, dict[str, Any]] = {}
        for token, data in refresh_tokens.items():
            if not isinstance(token, str) or not isinstance(data, dict):
                continue
            client_id = data.get("client_id")
            resource = data.get("resource")
            issued_at = data.get("issued_at")
            if isinstance(client_id, str) and isinstance(resource, str):
                loaded_tokens[token] = {
                    "client_id": client_id,
                    "resource": resource,
                    "issued_at": issued_at if isinstance(issued_at, int) else int(time.time()),
                }
        with config.refresh_tokens_lock:
            config.refresh_tokens.update(loaded_tokens)


def save_oauth_state(config: OAuthConfig) -> None:
    if not config.state_file:
        return
    with config.refresh_tokens_lock:
        refresh_tokens = dict(config.refresh_tokens)
    state = {
        "clients": config.registry.to_jsonable(),
