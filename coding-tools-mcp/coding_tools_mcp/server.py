...
WINDOWS_CORE_ENV_CANONICAL_NAMES = {
    "PATH": "Path",
    "PATHEXT": "PATHEXT",
    "COMSPEC": "ComSpec",
...
REMOTE_PROFILE_RE = re.compile(r"^[A-Za-z0-9_.-]+$")
...
def canonical_command_env_name(name: str) -> str:
    if os.name != "nt":
        return name
    return WINDOWS_CORE_ENV_CANONICAL_NAMES.get(name.upper(), name)


@functools.lru_cache(maxsize=1)
def windows_bash_executable() -> str | None:
    if os.name != "nt":
        return None
    configured = os.environ.get(f"{ENV_PREFIX}_BASH")
    candidates = [
        configured,
        shutil.which("bash"),
...
def bash_compatible_windows_command(command: str) -> str:
    def replace(match: re.Match[str]) -> str:
        tail = match.group(2).replace("\\", "/")
        return f"{match.group(1)}:/{tail}"

    return WINDOWS_DRIVE_PATH_RE.sub(replace, command)


def windows_bash_command_argv(command: str) -> list[str] | None:
    bash = windows_bash_executable()
    if bash is None:
        return None
    return [bash, "--noprofile", "--norc", "-lc", bash_compatible_windows_command(command)]
...
TOOL_REGISTRY: dict[str, ToolSpec] = {
    ...
    "write_file": ToolSpec(
        title="Write file",
        description="Write UTF-8 text to a file in the active target.",
        destructive=True,
        error_status="failed",
    ),
    "list_dir": ToolSpec(
    ...
    "remote_list_profiles": ToolSpec(
        title="List remote profiles",
        description="List local windows-remote-server profile names available for SSH/SFTP remote mode.",
        read_only=True,
        idempotent=True,
    ),
    "remote_exec": ToolSpec(
        title="Remote exec",
        description="Run a bounded shell command on a remote server through a local windows-remote-server profile.",
        destructive=True,
        open_world=True,
        error_status="failed",
    ),
    "remote_list_dir": ToolSpec(
        title="Remote list directory",
        description="List a directory on a remote server through a local windows-remote-server profile.",
        read_only=True,
        idempotent=True,
        open_world=True,
    ),
    "remote_read_file": ToolSpec(
        title="Remote read file",
        description="Read a UTF-8 text file from a remote server through a local windows-remote-server profile.",
        read_only=True,
        idempotent=True,
        open_world=True,
    ),
    "remote_list_files": ToolSpec(
        title="Remote list files",
        description="List files on a remote server through a local windows-remote-server profile.",
        read_only=True,
        idempotent=True,
        open_world=True,
    ),
    "remote_search_text": ToolSpec(
        title="Remote search text",
        description="Search remote server files for text or regex matches through a local windows-remote-server profile.",
        read_only=True,
        idempotent=True,
        open_world=True,
    ),
    "remote_write_file": ToolSpec(
        title="Remote write file",
        description="Write UTF-8 text to a remote server file through a local windows-remote-server profile.",
        destructive=True,
        open_world=True,
        error_status="failed",
    ),
    "remote_apply_patch": ToolSpec(
        title="Remote apply patch",
        description="Apply a patch envelope to remote server text files through a local windows-remote-server profile.",
        destructive=True,
        open_world=True,
        error_status="failed",
    ),
    "remote_git_status": ToolSpec(
        title="Remote git status",
        description="Return git working tree status on a configured remote server.",
        read_only=True,
        idempotent=True,
        open_world=True,
    ),
    "remote_git_diff": ToolSpec(
        title="Remote git diff",
        description="Return bounded git diff output on a configured remote server.",
        read_only=True,
        idempotent=True,
        open_world=True,
    ),
    "remote_git_log": ToolSpec(
        title="Remote git log",
        description="Return bounded git history on a configured remote server.",
        read_only=True,
        idempotent=True,
        open_world=True,
    ),
    "remote_git_show": ToolSpec(
        title="Remote git show",
        description="Return bounded git show output on a configured remote server.",
        read_only=True,
        idempotent=True,
        open_world=True,
    ),
    "remote_git_blame": ToolSpec(
        title="Remote git blame",
        description="Return bounded git blame metadata on a configured remote server.",
        read_only=True,
        idempotent=True,
        open_world=True,
    ),
    "remote_upload": ToolSpec(
        title="Remote upload",
        description="Upload a local workspace file or directory to a remote server through SFTP.",
        destructive=True,
        open_world=True,
        error_status="failed",
    ),
    "remote_download": ToolSpec(
        title="Remote download",
        description="Download a remote server file or directory into the local workspace through SFTP.",
        destructive=True,
        open_world=True,
        error_status="failed",
    ),
...
def truncate_bytes(data: bytes, limit: int) -> tuple[str, bool]:
    ...
    if truncated:
        ...
        if limit > len(marker) + 2:
            ...
            data = data[:limit]
    return decode_output_bytes(data), truncated
...
class Runtime:
    def __init__(
        ...
        *,
        ...
        self._tool_handlers = {name: name for name in TOOL_REGISTRY}
    ...
    def _repo_runtime_dir(self) -> Path:
        return Path(__file__).resolve().parents[1] / ".runtime"

    def _default_active_target_payload(self) -> dict[str, Any]:
        return {
            "mode": "local",
    ...
    def _active_target_matches_workspace(self, path_text: str) -> bool:
        path_text = path_text.strip()
        if not path_text:
            return True
        try:
            candidate = Path(path_text).expanduser().resolve(strict=False)
            workspace = self.workspace.root.resolve(strict=False)
        except OSError:
            return False
        candidate_text = os.path.normcase(os.path.normpath(str(candidate)))
        workspace_text = os.path.normcase(os.path.normpath(str(workspace)))
        return candidate_text == workspace_text

    def active_target_payload(self) -> dict[str, Any]:
        path = self._repo_runtime_dir() / "chatgpt-mcp-active-target.json"
        if not path.is_file():
            return self._default_active_target_payload()
        try:
        ...
        if mode not in {"local", "remote", "unselected"}:
            mode = "local"
        host_workspace = str(raw.get("local_host_workspace_path") or raw.get("workspace_path") or "")
        if host_workspace and not self._active_target_matches_workspace(host_workspace):
            return self._default_active_target_payload()
        payload = {
            "mode": mode,
            "remote_profile": str(raw.get("remote_profile") or ""),
            "remote_folder": str(raw.get("remote_folder") or ""),
            "ssh_connection": str(raw.get("ssh_connection") or ""),
            "updated_at": str(raw.get("updated_at") or ""),
        }
        if mode == "remote":
            payload["local_host_workspace_path"] = str(raw.get("local_host_workspace_path") or raw.get("workspace_path") or self.workspace.root)
        elif mode == "unselected":
            payload["local_host_workspace_path"] = str(raw.get("local_host_workspace_path") or raw.get("workspace_path") or self.workspace.root)
        else:
            payload["workspace_path"] = str(raw.get("workspace_path") or self.workspace.root)
        return payload

    def active_target_instruction(self) -> str:
        target = self.active_target_payload()
        if target.get("mode") == "remote":
            profile = target.get("remote_profile") or ""
            folder = target.get("remote_folder") or ""
            return (
                "Coding MCP active target is REMOTE, not the local Windows host folder. "
                f"Treat unqualified paths as files on profile={profile!r} under remote_dir={folder!r}. "
                "When the user asks what is connected, answer with the remote profile and remote directory only. "
                "Use local host paths only for MCP hosting/staging, or when the user explicitly asks for local files."
            )
        if target.get("mode") == "unselected":
            return (
                "Coding MCP active target is NOT SELECTED for this run. "
                "Do not read, write, list, search, or execute against local or remote files until the user chooses "
                "Use Local Folder or Use Remote Folder in the launcher."
            )
        workspace = target.get("workspace_path") or str(self.workspace.root)
        return (
            "Coding MCP active target: LOCAL folder. "
            f"Use local workspace tools by default under workspace={workspace!r}. "
            "Use remote_* tools only when the user explicitly asks for a server profile."
        )

    def initialize(self) -> dict[str, Any]:
        instructions = self.project_context.server_instructions()
        target_instruction = self.active_target_instruction()
        if target_instruction:
            instructions = f"{instructions}\n\n{target_instruction}" if instructions else target_instruction
        return {
    ...
    def _target_workspace_summary(
        self,
        *,
        target: dict[str, Any] | None = None,
        exec_summary: dict[str, Any] | None = None,
    ) -> dict[str, Any]:
        target = target or self.active_target_payload()
        summary = dict(exec_summary or self._exec_environment_summary())
        local_workspace = str(summary.pop("workspace", self.workspace.root))
        display_target = dict(target)
        display_target.pop("local_host_workspace_path", None)
        if target.get("mode") == "remote":
            profile = str(target.get("remote_profile") or "")
            remote_folder = str(target.get("remote_folder") or "")
            effective = f"{profile}:{remote_folder or '.'}"
            summary.update(
                {
                    "active_target": display_target,
                    "effective_workspace": effective,
                    "default_cwd": effective,
                    "host_workspace_hidden": True,
                    "host_workspace_note": "Local Windows host workspace is hidden while a remote target is active.",
                }
            )
        elif target.get("mode") == "unselected":
            summary.update(
                {
                    "active_target": display_target,
                    "effective_workspace": "UNSELECTED",
                    "default_cwd": "UNSELECTED",
                    "host_workspace_hidden": True,
                    "host_workspace_note": "No active coding target is selected for this run.",
                }
            )
        else:
            workspace = str(target.get("workspace_path") or local_workspace)
            summary.update(
                {
    ...
    def server_info_payload(self) -> dict[str, Any]:
        ...
        target = self.active_target_payload()
        workspace_summary = self._target_workspace_summary(target=target)
        payload = {
            "server": SERVER_NAME,
            "title": "Coding Tools MCP",
            "version": __version__,
    ...
    def _remote_target_routed_call(self, name: str, args: dict[str, Any]) -> tuple[str, dict[str, Any]]:
        status_tools = {
            "server_info",
        ...
        if name.startswith("remote_") or name in status_tools:
            return name, args
        target = self.active_target_payload()
        if target.get("mode") == "unselected":
            raise ToolFailure(
                "TARGET_NOT_SELECTED",
                "No active target is selected for this run.",
                category="validation",
                details={
                    "retry_hint": "Open the launcher, fill the fields for this run, then click Use Local Folder or Use Remote Folder.",
                },
            )
        if target.get("mode") != "remote":
            return name, args
        profile = str(target.get("remote_profile") or "").strip()
        remote_dir = str(target.get("remote_folder") or "").strip()
        if not profile:
            raise ToolFailure(
                "REMOTE_TARGET_NOT_CONFIGURED",
                "Active target is remote, but no remote profile is configured.",
                category="validation",
                details={"retry_hint": "Open the launcher, fill SSH connection/profile/folder, then click Use Remote Folder."},
            )
        ...
        if name == "read_file":
            ...
            if args.get("end_line") is not None:
                routed["end_line"] = int(args["end_line"])
            return "remote_read_file", routed
        if name == "write_file":
            routed = {
                "profile": profile,
                "path": str(args.get("path", "")),
            ...
            return "remote_write_file", routed
        if name == "list_files":
            routed = {
                "profile": profile,
                "path": str(args.get("path", ".")),
                ...
                "sort": str(args.get("sort", "path")),
                "timeout_ms": int(args.get("timeout_ms", 30000)),
            }
            if isinstance(args.get("glob"), str):
                routed["glob"] = str(args["glob"])
            return "remote_list_files", routed
        if name == "search_text":
            routed = {
                "profile": profile,
            ...
            if isinstance(args.get("glob"), str):
                routed["glob"] = str(args["glob"])
            return "remote_search_text", routed
        if name == "apply_patch":
            routed = {
                "profile": profile,
            ...
            return "remote_apply_patch", routed
        if name == "exec_command":
            routed = {
                ...
                "pty": bool(args.get("tty", False)),
            }
            return "remote_exec", routed
        remote_git_tools = {
            "git_status": "remote_git_status",
            "git_diff": "remote_git_diff",
            "git_log": "remote_git_log",
            "git_show": "remote_git_show",
            "git_blame": "remote_git_blame",
        }
        if name in remote_git_tools:
            routed = dict(args)
            routed["profile"] = profile
            routed["remote_dir"] = remote_dir
            routed["timeout_ms"] = int(args.get("timeout_ms", 30000))
            return remote_git_tools[name], routed
        ...
        if name in blocked:
            raise ToolFailure(
                ...
                f"Active target is remote ({profile}:{remote_dir or '.'}); blocked local tool '{name}' to avoid touching local files.",
                category="validation",
                details={
                    "remote_profile": profile,
                    "remote_folder": remote_dir,
                    "retry_hint": "Use remote_* tools, or click Use Local Folder in the launcher before using local tools.",
                },
            )
        return name, args
    ...
    def call_tool(
        ...
        *,
        ...
        try:
            ...
            name, args = self._remote_target_routed_call(name, args)
            handler_name = self._tool_handlers.get(name) if name in self.exposed_tool_names() else None
            handler = getattr(self, handler_name, None) if handler_name is not None else None
            if handler is None:
                raise JsonRpcError(-32602, f"Unknown tool: {name}", {"reason": "unknown_tool", "original_tool": original_name})
            ...
            if spec is None:
                spec = TOOL_REGISTRY.get(name) or TOOL_REGISTRY.get(original_name)
    ...
    def check_exec_environment(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "ok": True,
            **self._target_workspace_summary(),
    ...
    def get_default_cwd(self, args: dict[str, Any]) -> dict[str, Any]:
        summary = self._target_workspace_summary()
        keys = (
            "workspace",
            "effective_workspace",
            "default_cwd",
            "active_target",
            "local_host_workspace",
            "local_host_default_cwd",
            "local_host_note",
        )
    ...
    def write_file(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        with self.patch_lock:
            self.workspace.reject_write_symlink(requested_path)
            target = self.resolve_for_write(requested_path)
            if target.path.exists() and target.path.is_dir():
                raise ToolFailure("IS_DIRECTORY", "Path is a directory.", category="validation")
            baseline = FileBaseline.capture(target.path)
            mode = baseline.mode
            staged = [StagedFile(target.display, target.path, content, baseline, mode)]
            if not dry_run:
        ...
        return {
            ...
            "created": baseline.data is None,
            "summary": f"{'Validated write' if dry_run else 'Wrote'} {target.display}",
            "warnings": [],
        }
    ...
    def exec_command(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        if os.name == "nt" and popen_shell:
            bash_argv = windows_bash_command_argv(cmd)
    ...
    def _session_output_summary(self, session: ExecSession, payload: dict[str, Any]) -> str:
        retained = decode_output_bytes(session.retained_output_bytes())
    ...
    def read_output(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        content_buffer_offset = buffer_offset
        if (
            content_buffer_offset > 0
            and data[content_buffer_offset - 1 : content_buffer_offset] == b"\r"
            and data[content_buffer_offset : content_buffer_offset + 1] == b"\n"
        ):
            content_buffer_offset += 1
        chunk = data[content_buffer_offset : content_buffer_offset + limit]
        raw_end_offset = retained_start_offset + content_buffer_offset + len(chunk)
        next_offset = raw_end_offset if raw_end_offset < total_stream_bytes else None
    ...
    def _remote_scripts_dir(self) -> Path:
        configured = os.environ.get(f"{ENV_PREFIX}_WINDOWS_REMOTE_SKILL_DIR") or ""
        if configured.strip():
            candidate = Path(configured).expanduser().resolve(strict=False)
            if candidate.name.lower() != "scripts":
                candidate = candidate / "scripts"
        else:
            candidate = self._portable_root() / "skills" / "windows-remote-server" / "scripts"
        required = ("invoke_remote.ps1", "transfer_remote.ps1")
        missing = [name for name in required if not (candidate / name).is_file()]
        if missing:
            raise ToolFailure(
                "REMOTE_SKILL_NOT_FOUND",
                "windows-remote-server skill scripts were not found.",
                category="runtime",
                details={
                    "scripts_dir": str(candidate),
                    "missing": missing,
                    "retry_hint": f"Set {ENV_PREFIX}_WINDOWS_REMOTE_SKILL_DIR to the windows-remote-server skill folder.",
                },
            )
        return candidate

    def _remote_store_root(self, args: dict[str, Any]) -> Path:
        raw_root = str(args.get("remote_root") or os.environ.get(f"{ENV_PREFIX}_REMOTE_ROOT") or ".codex-remote")
        root = Path(raw_root).expanduser()
        if not root.is_absolute():
            root = self._portable_root() / root
        return root.resolve(strict=False)

    def _validate_remote_profile(self, profile: Any) -> str:
        profile_name = str(profile or "").strip()
        if not profile_name:
            raise ToolFailure("INVALID_ARGUMENT", "profile is required.", category="validation")
        if not REMOTE_PROFILE_RE.fullmatch(profile_name):
            raise ToolFailure(
                "INVALID_ARGUMENT",
                "Invalid profile name. Use letters, digits, dot, underscore, or hyphen only.",
                category="validation",
            )
        return profile_name

    def _remote_profile_path(self, profile: str, root: Path) -> Path:
        return root / "profiles" / f"{profile}.env"

    def _read_remote_profile_env(self, profile: str, root: Path) -> dict[str, str]:
        path = self._remote_profile_path(profile, root)
        if not path.is_file():
            raise ToolFailure(
                "REMOTE_PROFILE_NOT_FOUND",
                f"Remote profile not found: {profile}",
                category="validation",
                details={
                    "profile": profile,
                    ...
                    "retry_hint": (
                        "Create it with "
                        f"powershell -ExecutionPolicy Bypass -File {self._remote_scripts_dir() / 'save_remote_profile.ps1'} "
                        f"-Profile {profile} -Root {root}"
                    ),
                },
            )
        values: dict[str, str] = {}
        try:
                ...
                for raw_line in handle:
                    ...
                    if not re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*", key):
                        raise ToolFailure(
                            ...
                            f"Invalid environment key in profile {profile}: {key}",
                            category="validation",
                        )
            ...
            raise ToolFailure(
                "REMOTE_PROFILE_UNREADABLE",
                f"Could not read remote profile {profile}: {exc}",
                category="runtime",
            ) from exc
        return values

    def _remote_profile_public_info(self, profile: str, root: Path) -> dict[str, Any]:
        env = self._read_remote_profile_env(profile, root)
        return {
            "profile": profile,
            "host": env.get("REMOTE_HOST", ""),
            "port": env.get("REMOTE_PORT", ""),
            "user": env.get("REMOTE_USER", ""),
            "remote_workdir": env.get("REMOTE_WORKDIR", ""),
            "auth": "key" if env.get("REMOTE_KEY") else "password" if env.get("REMOTE_CREDENTIAL_XML") else "agent_or_prompt",
        }

    def _remote_resolve_path(
        self,
        raw_path: str,
        *,
        ...
        if not path:
            raise ToolFailure("INVALID_ARGUMENT", "remote path is required.", category="validation")
        base = (remote_dir or profile_env.get("REMOTE_WORKDIR") or "").strip().replace("\\", "/")
        base = posixpath.normpath(base) if base and base != "." else ""
        if not base:
            raise ToolFailure(
                "INVALID_ARGUMENT",
                "Remote folder is required for Coding MCP remote tools. Choose a concrete project folder such as /path/to/project.",
                category="validation",
            )
        if base == "/":
            raise ToolFailure(
                "INVALID_ARGUMENT",
                "Remote folder '/' is not allowed as a Coding MCP target. Choose a concrete project folder such as /path/to/project.",
                category="validation",
            )
        if path.startswith("~"):
            raise ToolFailure(
                "PATH_OUTSIDE_WORKSPACE",
                "Remote home-relative paths are outside the configured remote folder.",
                category="security",
            )
        if path.startswith("/"):
        ...
        if base and not (resolved == base or resolved.startswith(base.rstrip("/") + "/")):
            raise ToolFailure(
                "PATH_OUTSIDE_WORKSPACE",
                "Remote path escapes the configured remote folder.",
                category="security",
                details={"remote_folder": base, "path": resolved},
            )
        return resolved

    def _remote_command(self, command: str, remote_dir: str | None) -> str:
        directory = (remote_dir or "").strip()
        if not directory or directory == ".":
            return command
        remote_directory = directory.replace("\\", "/")
        return f"cd {shlex.quote(remote_directory)} && {command}"
    ...
    def _run_remote_script(
        ...
        *,
        ...
        timeout_ms = int(args.get("timeout_ms", 30000))
        root = self._remote_store_root(args)
        script = self._remote_scripts_dir() / script_name
        powershell = shutil.which("powershell.exe") or shutil.which("powershell") or shutil.which("pwsh")
        if not powershell:
            raise ToolFailure(
                "POWERSHELL_NOT_FOUND",
                "PowerShell is required for windows-remote-server profile loading.",
                category="runtime",
            )
        cmd = [
            powershell,
        ...
        try:
            ...
            stdout = truncate_text_head(stdout_text, max_lines=DEFAULT_MAX_LINES, max_bytes=max_output_bytes)
            stderr = truncate_text_head(stderr_text, max_lines=DEFAULT_MAX_LINES, max_bytes=max_output_bytes)
            return {
                "status": "timeout",
                "timed_out": True,
                "exit_code": None,
                ...
                "warnings": ["remote command timed out"],
            }
        stdout = truncate_text_head(completed.stdout, max_lines=DEFAULT_MAX_LINES, max_bytes=max_output_bytes)
        stderr = truncate_text_head(completed.stderr, max_lines=DEFAULT_MAX_LINES, max_bytes=max_output_bytes)
        warnings: list[str] = []
        if stdout.truncated or stderr.truncated:
            warnings.append("output truncated")
        return {
            "status": "exited" if completed.returncode == 0 else "failed",
            "timed_out": False,
    ...
    def _remote_context(self, args: dict[str, Any]) -> tuple[str, Path, dict[str, str], str | None]:
        profile = self._validate_remote_profile(args.get("profile"))
        root = self._remote_store_root(args)
        profile_env = self._read_remote_profile_env(profile, root)
        remote_dir = str(args.get("remote_dir") or "").strip() or None
        return profile, root, profile_env, remote_dir

    def _remote_path_exists(self, args: dict[str, Any], profile: str, path: str) -> bool:
        payload = self._run_remote_script(
            "invoke_remote.ps1",
            args,
            ["-Profile", profile, "-Command", f"test -e {shlex.quote(path)}"],
            max_output_bytes=4096,
        )
        return payload.get("exit_code") == 0

    def _remote_read_patch_text(
        self,
        args: dict[str, Any],
        profile: str,
        path: str,
        *,
        ...
        payload = self._run_remote_script(
            "invoke_remote.ps1",
            args,
            ["-Profile", profile, "-Command", f"test -f {quoted} && cat -- {quoted}"],
            max_output_bytes=max_bytes,
        )
        if payload.get("exit_code") != 0:
            raise ToolFailure("NOT_FOUND", f"Remote file not found or not readable: {path}", category="not_found")
        if payload.get("stdout_truncated"):
            raise ToolFailure("OUTPUT_TOO_LARGE", f"Remote file is larger than {max_bytes} bytes: {path}", category="validation")
        return str(payload.get("stdout", ""))

    @staticmethod
    def _remote_payload_failed(payload: dict[str, Any]) -> bool:
        return payload.get("timed_out") is True or payload.get("exit_code") not in {0, None}

    def remote_list_profiles(self, args: dict[str, Any]) -> dict[str, Any]:
        root = self._remote_store_root(args)
        ...
        if profiles_dir.is_dir():
            for path in sorted(profiles_dir.glob("*.env")):
                profile = path.stem
                if REMOTE_PROFILE_RE.fullmatch(profile):
                    profiles.append(self._remote_profile_public_info(profile, root))
        return {
            "remote_root": str(root),
            "profiles": profiles,
            "count": len(profiles),
            "warnings": [] if profiles else ["no remote profiles found"],
        }

    def remote_exec(self, args: dict[str, Any]) -> dict[str, Any]:
        profile, root, profile_env, remote_dir = self._remote_context(args)
        command = str(args.get("command", ""))
        if not command:
            raise ToolFailure("INVALID_ARGUMENT", "command is required.", category="validation")
        checked_remote_dir = self._remote_resolve_path(".", profile_env=profile_env, remote_dir=remote_dir)
        effective_command = self._remote_command(command, checked_remote_dir)
        script_args = ["-Profile", profile, "-Command", effective_command]
        if bool(args.get("pty", False)):
            script_args.append("-Pty")
        payload = self._run_remote_script(
            "invoke_remote.ps1",
            args,
            script_args,
    ...
    def remote_list_dir(self, args: dict[str, Any]) -> dict[str, Any]:
        profile, root, profile_env, remote_dir = self._remote_context(args)
        path = self._remote_resolve_path(str(args.get("path", ".")), profile_env=profile_env, remote_dir=remote_dir)
        max_entries = int(args.get("max_entries", 500))
        command = f"ls -la -- {shlex.quote(path)} | head -n {max_entries + 1}"
        payload = self._run_remote_script(
            "invoke_remote.ps1",
            args,
    ...
    def remote_read_file(self, args: dict[str, Any]) -> dict[str, Any]:
        profile, root, profile_env, remote_dir = self._remote_context(args)
        path = self._remote_resolve_path(str(args.get("path", "")), profile_env=profile_env, remote_dir=remote_dir)
        start_line = int(args.get("start_line", 1))
        ...
        if start_line > 1 or end_line is not None:
            end_expr = str(end_line) if end_line is not None else "$"
            command = f"sed -n {shlex.quote(f'{start_line},{end_expr}p')} -- {quoted_path} | head -c {max_bytes}"
        else:
            command = f"cat -- {quoted_path} | head -c {max_bytes}"
        payload = self._run_remote_script(
            "invoke_remote.ps1",
            args,
            ["-Profile", profile, "-Command", command],
        ...
        payload.update(
            {
                ...
                "remote_dir": remote_dir,
                "path": path,
                "content": payload.get("stdout", ""),
                "start_line": start_line,
                "end_line": end_line,
                "truncated": bool(payload.get("stdout_truncated")),
            }
        )
        return payload

    def remote_list_files(self, args: dict[str, Any]) -> dict[str, Any]:
        profile, root, profile_env, remote_dir = self._remote_context(args)
        path = self._remote_resolve_path(str(args.get("path", ".")), profile_env=profile_env, remote_dir=remote_dir)
        patterns_arg = args.get("patterns")
        glob_arg = args.get("glob")
        ...
        include_ignored = bool(args.get("include_ignored", False))
        max_results = int(args.get("max_results", 5000))
        remote_limit = max(max_results * 4, max_results + 100)
        quoted_path = shlex.quote(path)
        command = (
            f"if test -d {quoted_path}; then "
            f"find {quoted_path} -mindepth 1 \\( -type f -o -type l \\) -printf '%y\\t%s\\t%T@\\t%p\\n' | head -n {remote_limit}; "
            f"else find {quoted_path} -maxdepth 0 \\( -type f -o -type l \\) -printf '%y\\t%s\\t%T@\\t%p\\n'; fi"
        )
        payload = self._run_remote_script(
            "invoke_remote.ps1",
            args,
        ...
        files: list[dict[str, Any]] = []
        truncated = False
        for raw_line in str(payload.get("stdout", "")).splitlines():
            kind, size_text, modified_text, absolute = (raw_line.split("\t", 3) + ["", "", "", ""])[:4]
            if not absolute:
                continue
            rel = posixpath.relpath(absolute, path)
            if rel == ".":
            ...
            if any(fnmatch.fnmatch(rel, pattern) or PurePosixPath(rel).match(pattern) for pattern in exclude_patterns):
                continue
            try:
                modified = datetime.fromtimestamp(float(modified_text), timezone.utc).isoformat().replace("+00:00", "Z")
            except ValueError:
                modified = ""
            files.append(
                {
                    "path": rel,
                    "type": "symlink" if kind == "l" else "file",
                    "size_bytes": int(float(size_text or "0")),
                    "modified": modified,
                }
            )
            if len(files) >= max_results:
        ...
        payload.update(
            {
                ...
                "path": path,
                "files": files,
                "truncated": truncated or bool(payload.get("stdout_truncated")),
                "warnings": warnings + (["result limit reached"] if truncated else []),
            }
        )
        return payload

    def remote_search_text(self, args: dict[str, Any]) -> dict[str, Any]:
        profile, root, profile_env, remote_dir = self._remote_context(args)
        query = str(args.get("query", ""))
        if not query:
            raise ToolFailure("INVALID_ARGUMENT", "query is required.", category="validation")
        ...
        max_results = int(args.get("max_results", 1000))
        max_preview_bytes = int(args.get("max_preview_bytes", 512))
        rg_parts = ["rg", "--line-number", "--column", "--with-filename", "--no-heading", "--color", "never"]
        if not regex:
            rg_parts.append("--fixed-strings")
        if not case_sensitive:
            rg_parts.append("--ignore-case")
        for pattern in include_globs:
            rg_parts.extend(["-g", pattern])
        for pattern in exclude_globs:
            rg_parts.extend(["-g", f"!{pattern}"])
        rg_parts.extend(["--", query, path])
        rg_command = " ".join(shlex.quote(part) for part in rg_parts)
        grep_parts = ["grep", "-RInI"]
        grep_parts.append("-E" if regex else "-F")
        if not case_sensitive:
            grep_parts.append("-i")
        grep_parts.extend(["--", query, path])
        grep_command = " ".join(shlex.quote(part) for part in grep_parts)
        command = (
            "if command -v rg >/dev/null 2>&1; then "
            f"{rg_command} | head -n {max_results}; "
            "else "
            f"{grep_command} | head -n {max_results}; "
            "fi"
        )
        payload = self._run_remote_script(
            "invoke_remote.ps1",
        ...
        matches: list[dict[str, Any]] = []
        for raw_line in str(payload.get("stdout", "")).splitlines():
            first, second, third, rest = (raw_line.split(":", 3) + ["", "", "", ""])[:4]
            ...
            if preview_truncation.truncated:
                item["preview_truncated"] = True
                item["preview_truncated_by"] = preview_truncation.truncated_by
            matches.append(item)
        ...
        payload.update(
            {
                ...
                "total_matches_exact": not payload.get("stdout_truncated"),
                "truncated": bool(payload.get("stdout_truncated")) or len(matches) >= max_results,
            }
        )
        return payload

    def remote_write_file(self, args: dict[str, Any]) -> dict[str, Any]:
        profile, root, profile_env, remote_dir = self._remote_context(args)
        path = self._remote_resolve_path(str(args.get("path", "")), profile_env=profile_env, remote_dir=remote_dir)
        content = str(args.get("content", ""))
        self._ensure_runtime_dirs()
        ...
        try:
            ...
            if parent and parent not in {".", "/"}:
                mkdir_payload = self._run_remote_script(
                    "invoke_remote.ps1",
                    args,
                    ["-Profile", profile, "-Command", f"mkdir -p -- {shlex.quote(parent)}"],
                    max_output_bytes=65536,
                )
                if self._remote_payload_failed(mkdir_payload):
                    mkdir_payload.update(
                        {
                            "profile": profile,
                            "remote_root": str(root),
                            "remote_dir": remote_dir,
                            "path": path,
                            "bytes_written": 0,
                            "warnings": (mkdir_payload.get("warnings") if isinstance(mkdir_payload.get("warnings"), list) else [])
                            + ["remote parent directory could not be prepared"],
                        }
                    )
                    return mkdir_payload
            payload = self._run_remote_script(
                "transfer_remote.ps1",
                args,
                ["-Profile", profile, "-Action", "upload", "-Local", str(temp_path), "-Remote", path],
                max_output_bytes=65536,
            )
        finally:
    ...
    def remote_apply_patch(self, args: dict[str, Any]) -> dict[str, Any]:
        profile, root, profile_env, remote_dir = self._remote_context(args)
        patch = str(args.get("patch", ""))
        dry_run = bool(args.get("dry_run", False))
        operations = parse_patch(patch)
        ...
        removals = 0
        for op in operations:
            path = self._remote_resolve_path(op.path, profile_env=profile_env, remote_dir=remote_dir)
            if op.kind == "add":
                if self._remote_path_exists(args, profile, path):
                    raise ToolFailure("PATCH_FAILED", "Cannot add remote file that already exists.", category="validation")
                content = op.add_content or ""
                if not dry_run:
                    write_payload = self.remote_write_file(
                        {
                            "profile": profile,
                            "path": op.path,
                            "content": content,
                    ...
                    if self._remote_payload_failed(write_payload):
                        raise ToolFailure("PATCH_FAILED", f"Remote write failed for {op.path}", category="runtime")
                affected.append({"path": path, "operation": "add"})
                summaries.append(f"A {path}")
                additions += len(content.splitlines())
            elif op.kind == "delete":
                if not self._remote_path_exists(args, profile, path):
                    raise ToolFailure("NOT_FOUND", f"Remote file not found: {path}", category="not_found")
                if not dry_run:
                    payload = self._run_remote_script(
                        "invoke_remote.ps1",
                        args,
                        ["-Profile", profile, "-Command", f"rm -f -- {shlex.quote(path)}"],
                        max_output_bytes=65536,
                    )
                    if self._remote_payload_failed(payload):
                        raise ToolFailure("PATCH_FAILED", f"Remote delete failed for {path}", category="runtime")
                affected.append({"path": path, "operation": "delete"})
                summaries.append(f"D {path}")
            elif op.kind == "update":
                content = self._remote_read_patch_text(args, profile, path)
                updated = apply_update_hunks(content, op.hunks, op.path)
                for hunk in op.hunks:
                    for line in hunk:
                        additions += line.startswith("+")
                        removals += line.startswith("-")
                if op.move_to:
                    dest = self._remote_resolve_path(op.move_to, profile_env=profile_env, remote_dir=remote_dir)
                    if dest != path and self._remote_path_exists(args, profile, dest):
                        raise ToolFailure("PATCH_FAILED", "Cannot move over an existing remote file.", category="validation")
                    if not dry_run:
                        write_payload = self.remote_write_file(
                            {
                                "profile": profile,
                        ...
                        if self._remote_payload_failed(write_payload):
                            raise ToolFailure("PATCH_FAILED", f"Remote write failed for {op.move_to}", category="runtime")
                        if dest != path:
                            delete_payload = self._run_remote_script(
                                "invoke_remote.ps1",
                                args,
                                ["-Profile", profile, "-Command", f"rm -f -- {shlex.quote(path)}"],
                                max_output_bytes=65536,
                            )
                            if self._remote_payload_failed(delete_payload):
                                raise ToolFailure("PATCH_FAILED", f"Remote source delete failed for {path}", category="runtime")
                    affected.append({"path": dest, "old_path": path, "operation": "move"})
                    summaries.append(f"R {path} -> {dest}")
                else:
                    if not dry_run:
                        write_payload = self.remote_write_file(
                            {
                        ...
                        if self._remote_payload_failed(write_payload):
                            raise ToolFailure("PATCH_FAILED", f"Remote write failed for {op.path}", category="runtime")
                    affected.append({"path": path, "operation": "update"})
                    summaries.append(f"M {path}")
        if not affected:
            raise ToolFailure("PATCH_FAILED", "No files were modified.", category="validation")
        return {
            "dry_run": dry_run,
            ...
            "additions": additions,
            "removals": removals,
            "warnings": ["remote patch is non-transactional across multiple files"],
        }

    def remote_git_status(self, args: dict[str, Any]) -> dict[str, Any]:
        profile, root, profile_env, remote_dir = self._remote_context(args)
        path = self._remote_resolve_path(str(args.get("path", ".")), profile_env=profile_env, remote_dir=remote_dir)
        max_entries = int(args.get("max_entries", 1000))
        include_untracked = bool(args.get("include_untracked", True))
        untracked = "" if include_untracked else " --untracked-files=no"
        quoted = shlex.quote(path)
        command = (
            f"git -C {quoted} rev-parse --show-toplevel >/dev/null 2>&1 || exit 2; "
            f"git -C {quoted} rev-parse HEAD; printf '\\036'; "
            f"git -C {quoted} status --porcelain=v1 -b{untracked}"
        )
        payload = self._run_remote_script("invoke_remote.ps1", args, ["-Profile", profile, "-Command", command], max_output_bytes=262144)
        if payload.get("exit_code") == 2:
            payload.update({"is_repo": False, "entries": [], "warnings": ["not a git repository"]})
            return payload
        if self._remote_payload_failed(payload):
            raise ToolFailure("GIT_ERROR", str(payload.get("stderr") or "remote git status failed").strip(), category="runtime")
        head, _, status_text = str(payload.get("stdout", "")).partition("\x1e")
        branch = ""
        upstream = ""
        ahead = 0
        behind = 0
        ...
        for line in lines:
            ...
            if " -> " in path_text:
                original, path_text = path_text.split(" -> ", 1)
            entries.append({"path": path_text, "original_path": original, "index_status": line[0], "worktree_status": line[1]})
            if len(entries) >= max_entries:
                break
        payload.update(
            {
    ...
    def remote_git_diff(self, args: dict[str, Any]) -> dict[str, Any]:
        profile, root, profile_env, remote_dir = self._remote_context(args)
        cwd = self._remote_resolve_path(".", profile_env=profile_env, remote_dir=remote_dir)
        staged = bool(args.get("staged", False))
        unstaged = bool(args.get("unstaged", True))
        context = int(args.get("context_lines", 3))
        max_bytes = int(args.get("max_bytes", 262144))
        path_filters: list[str] = []
        if isinstance(args.get("path"), str):
            path_filters.append(str(args["path"]).replace("\\", "/"))
        if isinstance(args.get("paths"), list):
            path_filters.extend(str(item).replace("\\", "/") for item in args["paths"])
        pathspec = (" -- " + " ".join(shlex.quote(path) for path in path_filters)) if path_filters else ""
        quoted = shlex.quote(cwd)
        chunks = []
        if unstaged:
            chunks.append(f"git -C {quoted} diff --no-ext-diff --unified={context}{pathspec}")
        if staged:
            chunks.append(f"git -C {quoted} diff --cached --no-ext-diff --unified={context}{pathspec}")
        command = "; printf '\\n'; ".join(chunks) if chunks else "true"
        payload = self._run_remote_script("invoke_remote.ps1", args, ["-Profile", profile, "-Command", command], max_output_bytes=max_bytes)
        if self._remote_payload_failed(payload):
            raise ToolFailure("GIT_ERROR", str(payload.get("stderr") or "remote git diff failed").strip(), category="runtime")
        diff_text = str(payload.get("stdout", ""))
        payload.update(
            {
                "profile": profile,
                "remote_root": str(root),
                ...
                "files": parse_diff_files(diff_text),
                "truncated": bool(payload.get("stdout_truncated")),
                "warnings": ["diff truncated"] if payload.get("stdout_truncated") else [],
            }
        )
        return payload

    def remote_git_log(self, args: dict[str, Any]) -> dict[str, Any]:
        profile, root, profile_env, remote_dir = self._remote_context(args)
        cwd = self._remote_resolve_path(".", profile_env=profile_env, remote_dir=remote_dir)
        ref = validate_git_ref(str(args.get("ref", "HEAD")))
        max_count = int(args.get("max_count", 20))
        skip = int(args.get("skip", 0))
        path_filter = str(args.get("path", ".")).replace("\\", "/")
        pathspec = "" if path_filter in {"", "."} else f" -- {shlex.quote(path_filter)}"
        pretty = "--pretty=format:%H%x1f%h%x1f%an%x1f%ae%x1f%ad%x1f%s%x1e"
        command = (
            f"git -C {shlex.quote(cwd)} log --max-count={max_count + 1} --skip={skip} --date=iso-strict "
            f"{shlex.quote(pretty)} {shlex.quote(ref)}{pathspec}"
        )
        payload = self._run_remote_script("invoke_remote.ps1", args, ["-Profile", profile, "-Command", command], max_output_bytes=262144)
        if self._remote_payload_failed(payload):
            raise ToolFailure("GIT_ERROR", str(payload.get("stderr") or "remote git log failed").strip(), category="runtime")
        commits: list[dict[str, Any]] = []
        for record in str(payload.get("stdout", "")).split("\x1e"):
            fields = record.strip("\n").split("\x1f")
            if len(fields) < 6 or not fields[0]:
                continue
            commits.append(
    ...
    def remote_git_show(self, args: dict[str, Any]) -> dict[str, Any]:
        profile, root, profile_env, remote_dir = self._remote_context(args)
        cwd = self._remote_resolve_path(".", profile_env=profile_env, remote_dir=remote_dir)
        rev = validate_git_ref(str(args.get("rev", "HEAD")))
        context = int(args.get("context_lines", 3))
        ...
        if isinstance(args.get("paths"), list):
            path_filters.extend(str(item).replace("\\", "/") for item in args["paths"])
        pathspec = (" -- " + " ".join(shlex.quote(path) for path in path_filters)) if path_filters else ""
        no_patch = "" if include_diff else " --no-patch"
        command = (
            f"git -C {shlex.quote(cwd)} show --no-ext-diff --format=fuller --unified={context}"
            f"{no_patch} {shlex.quote(rev)}{pathspec}"
        )
        payload = self._run_remote_script("invoke_remote.ps1", args, ["-Profile", profile, "-Command", command], max_output_bytes=max_bytes)
        if self._remote_payload_failed(payload):
            raise ToolFailure("GIT_ERROR", str(payload.get("stderr") or "remote git show failed").strip(), category="runtime")
        content = str(payload.get("stdout", ""))
        payload.update(
            {
                "profile": profile,
                "remote_root": str(root),
                ...
                "files": parse_diff_files(content),
                "truncated": bool(payload.get("stdout_truncated")),
                "warnings": ["output truncated"] if payload.get("stdout_truncated") else [],
            }
        )
        return payload

    def remote_git_blame(self, args: dict[str, Any]) -> dict[str, Any]:
        profile, root, profile_env, remote_dir = self._remote_context(args)
        cwd = self._remote_resolve_path(".", profile_env=profile_env, remote_dir=remote_dir)
        requested_path = str(args.get("path", "")).replace("\\", "/")
        if not requested_path:
            raise ToolFailure("INVALID_ARGUMENT", "path is required.", category="validation")
        ref_arg = args.get("rev")
        ref = validate_git_ref(str(ref_arg)) if isinstance(ref_arg, str) and ref_arg else None
        ...
        end_line = args.get("end_line")
        max_lines = int(args.get("max_lines", 200))
        requested_final_line = start_line + max_lines - 1 if end_line is None else int(end_line)
        if requested_final_line < start_line:
            raise ToolFailure("INVALID_ARGUMENT", "end_line must be >= start_line.", category="validation")
        requested_lines = requested_final_line - start_line + 1
        truncated = requested_lines > max_lines
        final_line = min(requested_final_line, start_line + max_lines - 1)
        ref_part = f" {shlex.quote(ref)}" if ref else ""
        command = (
            f"git -C {shlex.quote(cwd)} blame --line-porcelain -L {start_line},{final_line}"
            f"{ref_part} -- {shlex.quote(requested_path)}"
        )
        payload = self._run_remote_script("invoke_remote.ps1", args, ["-Profile", profile, "-Command", command], max_output_bytes=262144)
        if self._remote_payload_failed(payload):
            raise ToolFailure("GIT_ERROR", str(payload.get("stderr") or "remote git blame failed").strip(), category="runtime")
        lines = parse_git_blame_porcelain(str(payload.get("stdout", "")))
        if len(lines) > max_lines:
            lines = lines[:max_lines]
            truncated = True
        payload.update(
    ...
    def remote_upload(self, args: dict[str, Any]) -> dict[str, Any]:
        profile = self._validate_remote_profile(args.get("profile"))
        root = self._remote_store_root(args)
        profile_env = self._read_remote_profile_env(profile, root)
        local = self.resolve_existing(str(args.get("local_path", "")))
        remote_dir = str(args.get("remote_dir") or "").strip() or None
        remote_path = self._remote_resolve_path(
            str(args.get("remote_path", "")),
        ...
        payload = self._run_remote_script(
            "transfer_remote.ps1",
            args,
            ["-Profile", profile, "-Action", "upload", "-Local", str(local.path), "-Remote", remote_path],
            max_output_bytes=65536,
        )
        payload.update(
            {
                ...
                "remote_root": str(root),
                "remote_dir": remote_dir,
                "local_path": local.display,
                "remote_path": remote_path,
            }
        )
        return payload

    def remote_download(self, args: dict[str, Any]) -> dict[str, Any]:
        profile = self._validate_remote_profile(args.get("profile"))
        root = self._remote_store_root(args)
        profile_env = self._read_remote_profile_env(profile, root)
        local = self.resolve_for_write(str(args.get("local_path", "")))
        remote_dir = str(args.get("remote_dir") or "").strip() or None
        remote_path = self._remote_resolve_path(
            str(args.get("remote_path", "")),
            profile_env=profile_env,
        ...
        payload = self._run_remote_script(
            "transfer_remote.ps1",
            args,
            ["-Profile", profile, "-Action", "download", "-Remote", remote_path, "-Local", str(local.path)],
            max_output_bytes=65536,
        )
        payload.update(
            {
...
def root_text_is_absolute(root: str) -> bool:
...
def guard_allow_roots() -> list[str]:
    ...
    for item in os.environ.get("PATH", "").split(os.pathsep):
        ...
        try:
            ...
            continue
        if path_is_dir(resolved) and is_default_system_path_root(resolved):
    ...
    for item in os.environ.get(f"{ENV_PREFIX}_EXEC_ALLOW_ROOTS", "").split(os.pathsep):
        ...
        if path_is_dir(resolved):
            roots.add(str(resolved))
    return sorted(root for root in roots if root and root_text_is_absolute(root))
...
def input_schemas() -> dict[str, dict[str, Any]]:
    ...
    return {
        ...
        "remote_list_profiles": object_schema(
            {
                "remote_root": string,
            }
        ),
        "remote_exec": object_schema(
            {
                "profile": {**string, "minLength": 1},
        ...
        "remote_list_dir": object_schema(
            {
                "profile": {**string, "minLength": 1},
                "path": {**string, "default": "."},
                "remote_dir": string,
                "max_entries": {**integer, "minimum": 1, "maximum": 10000, "default": 500},
                "timeout_ms": {**integer, "minimum": 1, "maximum": 600000, "default": 30000},
                "remote_root": string,
            },
            ["profile"],
        ),
        "remote_read_file": object_schema(
            {
                "profile": {**string, "minLength": 1},
                "path": {**string, "minLength": 1},
                "remote_dir": string,
        ...
        "remote_list_files": object_schema(
            {
                "profile": {**string, "minLength": 1},
                "path": {**string, "default": "."},
                "remote_dir": string,
        ...
        "remote_search_text": object_schema(
            {
                "profile": {**string, "minLength": 1},
                "query": {**string, "minLength": 1},
                "path": {**string, "default": "."},
        ...
        "remote_write_file": object_schema(
            {
                "profile": {**string, "minLength": 1},
                "path": {**string, "minLength": 1},
                "content": {**string},
        ...
        "remote_apply_patch": object_schema(
            {
                "profile": {**string, "minLength": 1},
        ...
        "remote_git_status": object_schema(
            {
                "profile": {**string, "minLength": 1},
                "path": {**string, "default": "."},
                "include_untracked": {**boolean, "default": True},
        ...
        "remote_git_blame": object_schema(
            {
                "profile": {**string, "minLength": 1},
                "path": {**string, "minLength": 1},
                "rev": string,
        ...
        "remote_upload": object_schema(
            {
                "profile": {**string, "minLength": 1},
                "local_path": {**string, "minLength": 1},
                "remote_path": {**string, "minLength": 1},
                "remote_dir": string,
                "timeout_ms": {**integer, "minimum": 1, "maximum": 600000, "default": 30000},
                "remote_root": string,
            },
...
class MCPHandler(http.server.BaseHTTPRequestHandler):
    ...
    def handle_oauth_token(self) -> None:
        ...
        if grant_type not in {"authorization_code", "refresh_token"}:
            _err("unsupported_grant_type", "Supported grants are authorization_code and refresh_token")
            return
        if grant_type not in client.grant_types:
            _err("unsupported_grant_type", "grant_type was not registered for this client")
            return

        def _store_refresh_token(server_url: str) -> str:
            new_refresh_token = secrets.token_urlsafe(48)
            with cfg.refresh_tokens_lock:
                while len(cfg.refresh_tokens) >= MAX_REFRESH_TOKENS:
                    oldest_token = next(iter(cfg.refresh_tokens))
                    cfg.refresh_tokens.pop(oldest_token, None)
                cfg.refresh_tokens[new_refresh_token] = {
                    "client_id": client_id,
                    "resource": server_url,
                    "issued_at": int(time.time()),
                }
            save_oauth_state(cfg)
        ...
        if grant_type == "refresh_token":
            if not refresh_token:
                ...
                return
            with cfg.refresh_tokens_lock:
                token_data = cfg.refresh_tokens.pop(refresh_token, None)
            if token_data is None:
                _err("invalid_grant", "Unknown or already-used refresh_token")
                return
            save_oauth_state(cfg)
            if not secrets.compare_digest(str(token_data.get("client_id") or ""), client_id):
                _err("invalid_grant", "client_id mismatch")
                return
            stored_resource = str(token_data.get("resource") or "").rstrip("/")
            if resource and not secrets.compare_digest(stored_resource, resource):
                _err("invalid_target", "resource mismatch")
                return
            server_url = resource or stored_resource
            if not server_url:
                _err("invalid_target", "resource is required")
                return
            access_token = create_access_token(cfg, server_url, client_id=client_id)
            self.send_json(
                {
                    "access_token": access_token,
                    "refresh_token": _store_refresh_token(server_url),
                    "token_type": "Bearer",
        ...
        server_url = resource
        access_token = create_access_token(cfg, server_url, client_id=client_id)
        response: dict[str, Any] = {"access_token": access_token, "token_type": "Bearer", "expires_in": cfg.token_ttl}
        if "refresh_token" in client.grant_types:
            response["refresh_token"] = _store_refresh_token(server_url)
        self.send_json(response)
...
def run_http(args: argparse.Namespace) -> int:
    ...
    if oauth_mode:
        ...
        oauth_config = OAuthConfig(
            ...
            token_ttl=token_ttl,
            state_file=os.environ.get(f"{ENV_PREFIX}_OAUTH_STATE_FILE") or None,
        )
        load_oauth_state(oauth_config)
        if client_id:
