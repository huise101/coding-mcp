...
"""
    ...
    def test_write_file_local_and_remote_routing(self) -> None:
        with TemporaryDirectory() as tmp:
            workspace = Path(tmp)
            runtime = Runtime(workspace)
            result = runtime.call_tool("write_file", {"path": "nested/内容.txt", "content": "牛"})
            payload = result["structuredContent"]
            self.assertEqual(payload.get("bytes_written"), len("牛".encode("utf-8")))
            self.assertEqual((workspace / "nested" / "内容.txt").read_text(encoding="utf-8"), "牛")

            captured: dict[str, object] = {}

            def fake_remote_write_file(args: dict[str, object]) -> dict[str, object]:
                captured.update(args)
                return {
                    "status": "exited",
                    "exit_code": 0,
                    "path": "/remote/workspace/远程目录/内容.txt",
                    "bytes_written": len(str(args["content"]).encode("utf-8")),
                }
            ...
            with (
                patch.object(
                    ...
                    return_value={"mode": "remote", "remote_profile": "s1", "remote_folder": "/remote/workspace"},
                ),
                patch.object(runtime, "remote_write_file", side_effect=fake_remote_write_file),
            ):
                result = runtime.call_tool("write_file", {"path": "远程目录/内容.txt", "content": "牛"})

            payload = result["structuredContent"]
            self.assertEqual(payload.get("path"), "/remote/workspace/远程目录/内容.txt")
            self.assertEqual(captured.get("profile"), "s1")
            self.assertEqual(captured.get("remote_dir"), "/remote/workspace")
            self.assertEqual(captured.get("path"), "远程目录/内容.txt")
            self.assertFalse((workspace / "远程目录").exists())

    def test_remote_target_routes_development_tools(self) -> None:
        with TemporaryDirectory() as tmp:
            runtime = Runtime(Path(tmp))
            with patch.object(
                runtime,
                ...
                cases = {
                    "list_files": ("remote_list_files", {"path": ".", "max_results": 5}),
                    "search_text": ("remote_search_text", {"query": "needle", "path": "."}),
                    "apply_patch": ("remote_apply_patch", {"patch": "*** Begin Patch\n*** End Patch"}),
                    "git_status": ("remote_git_status", {}),
                    "git_diff": ("remote_git_diff", {}),
                    "git_log": ("remote_git_log", {}),
                    "git_show": ("remote_git_show", {}),
                    "git_blame": ("remote_git_blame", {"path": "README.md"}),
                }
                for tool_name, (expected_name, args) in cases.items():
                    with self.subTest(tool_name=tool_name):
                        routed_name, routed_args = runtime._remote_target_routed_call(tool_name, args)
                        self.assertEqual(routed_name, expected_name)
                        self.assertEqual(routed_args.get("profile"), "s1")
                        self.assertEqual(routed_args.get("remote_dir"), "/remote/workspace")

    def test_remote_path_resolution_is_confined_to_remote_folder(self) -> None:
        with TemporaryDirectory() as tmp:
            runtime = Runtime(Path(tmp))
            profile_env = {"REMOTE_WORKDIR": "/remote/workspace"}
            self.assertEqual(
                runtime._remote_resolve_path("project/file.txt", profile_env=profile_env),
                "/remote/workspace/project/file.txt",
            )
            self.assertEqual(
                runtime._remote_resolve_path("/remote/workspace/project/file.txt", profile_env=profile_env),
                "/remote/workspace/project/file.txt",
            )
            for path in ("../outside.txt", "/etc/passwd", "~/secret"):
                with self.subTest(path=path):
                    with self.assertRaises(ToolFailure) as cm:
                        runtime._remote_resolve_path(path, profile_env=profile_env)
                    self.assertEqual(cm.exception.code, "PATH_OUTSIDE_WORKSPACE")
            with self.assertRaises(ToolFailure) as cm:
                runtime._remote_resolve_path("file.txt", profile_env={}, remote_dir="/")
            self.assertEqual(cm.exception.code, "INVALID_ARGUMENT")
            with self.assertRaises(ToolFailure) as cm:
                runtime._remote_resolve_path("/remote/workspace/file.txt", profile_env={})
            self.assertEqual(cm.exception.code, "INVALID_ARGUMENT")

    def test_remote_exec_rejects_root_remote_folder(self) -> None:
        with TemporaryDirectory() as tmp:
            runtime = Runtime(Path(tmp))
            with patch.object(runtime, "_remote_context", return_value=("s1", Path(tmp), {}, "/")):
                with self.assertRaises(ToolFailure) as cm:
                    runtime.remote_exec({"profile": "s1", "command": "pwd", "remote_dir": "/"})
            self.assertEqual(cm.exception.code, "INVALID_ARGUMENT")
