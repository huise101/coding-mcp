...
class MCPContractTests(ComplianceTestCase):
    ...
    def test_oauth_public_client_pkce_flow_succeeds(self) -> None:
        ...
        try:
            ...
            self.assertEqual(
                set(metadata.get("grant_types_supported", [])),
    ...
    def test_oauth_refresh_token_flow_succeeds(self) -> None:
        port = free_port()
        base_url = f"http://127.0.0.1:{port}"
        env = self.oauth_server_env(
            CODING_TOOLS_MCP_OAUTH_PASSWORD="test-password",
        ...
        try:
            ...
            registration = self.oauth_register(
                base_url,
                "Refresh Client",
                "none",
                grant_types=["authorization_code", "refresh_token"],
            )
            self.assertEqual(registration.get("grant_types"), ["authorization_code", "refresh_token"])
            ...
            refresh_status, refresh_response = self.oauth_refresh_token_request(
                base_url,
                client_id,
                str(refresh_token),
            )
            self.assertEqual(refresh_status, 200)
            self.assertIsInstance(refresh_response.get("access_token"), str)
            self.assertIsInstance(refresh_response.get("refresh_token"), str)

            ok_status, ok = self.raw_post_to_auth_server(
                f"{base_url}/mcp",
                token=refresh_response.get("access_token"),
            )
            self.assertEqual(ok_status, 200)
            self.assertEqual(ok.get("result"), {})

            reused_status, reused = self.oauth_refresh_token_request(base_url, client_id, str(refresh_token))
            self.assertEqual(reused_status, 400)
            self.assertEqual(reused.get("error"), "invalid_grant")
        finally:
            self.stop_process(process)

    def test_oauth_state_file_survives_server_restart(self) -> None:
        port = free_port()
        base_url = f"http://127.0.0.1:{port}"
        state_file = str(self.workspace.root / "oauth-state.json")
        env = self.oauth_server_env(
            CODING_TOOLS_MCP_OAUTH_PASSWORD="test-password",
            CODING_TOOLS_MCP_OAUTH_TOKEN_SECRET=bytes(range(32)).hex(),
            CODING_TOOLS_MCP_OAUTH_STATE_FILE=state_file,
        )
        process = self.start_oauth_server(port, env)
        try:
            self.wait_for_json(f"{base_url}/.well-known/oauth-authorization-server")
        ...
        try:
            ...
            refresh_status, refresh_response = self.oauth_refresh_token_request(
                base_url,
                client_id,
                str(token_response.get("refresh_token")),
            )
            self.assertEqual(refresh_status, 200)
            self.assertIsInstance(refresh_response.get("access_token"), str)
        finally:
    ...
    def oauth_refresh_token_request(
        self,
        base_url: str,
        client_id: str,
        refresh_token: str,
