...
The default catalog contains exactly 36 tools:
...
- `write_file`: write UTF-8 text to a local or active remote target file.
...
- `remote_list_profiles`: list locally saved SSH profile names.
- `remote_exec`: run a bounded command through a local encrypted remote profile.
- `remote_list_dir`: list a directory on a configured remote server.
- `remote_read_file`: read a UTF-8 file range from a configured remote server.
- `remote_list_files`: list remote files with glob, hidden-file, sort, and cap
  controls.
- `remote_search_text`: search remote files for literal or regex matches.
- `remote_write_file`: write UTF-8 content to a configured remote server.
- `remote_apply_patch`: apply add/update/delete/move patch envelopes to remote
  text files.
- `remote_git_status`: structured remote working-tree status.
- `remote_git_diff`: bounded remote unified staged/unstaged diff.
- `remote_git_log`: structured bounded remote commit history.
- `remote_git_show`: bounded remote revision metadata/content/diff.
- `remote_git_blame`: structured bounded remote line attribution.
- `remote_upload`: upload a local file or directory to a configured remote server.
- `remote_download`: download a remote file or directory to the local workspace.
...
content. That capability gate is not a tool profile. The other 35 tools are
