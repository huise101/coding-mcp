...
`apply_patch` and `write_file` are the direct text file-mutation primitives;
`edit_file` is not provided. Permission modes alter command policy, not the
advertised catalog.
...
- Remote path tools require a concrete `remote_dir` or profile `REMOTE_WORKDIR`.
  `/`, empty folders, `~`, absolute paths outside that folder, and `..`
  traversal outside that folder are rejected. `remote_exec` starts in the same
  checked folder; hard OS-level confinement still depends on the SSH account or
...
```json
["ABSOLUTE_PATH_DENIED", "BINARY_FILE", "ELICITATION_UNSUPPORTED", "GIT_ERROR", "INTERNAL_ERROR", "INVALID_ARGUMENT", "IS_DIRECTORY", "NOT_A_DIRECTORY", "NOT_FOUND", "OUTPUT_TOO_LARGE", "PATCH_CONFLICT", "PATCH_CONTEXT_AMBIGUOUS", "PATCH_CONTEXT_NOT_FOUND", "PATCH_FAILED", "PATCH_HUNKS_OVERLAP", "PATCH_ROLLBACK_FAILED", "PATH_OUTSIDE_WORKSPACE", "PERMISSION_REQUIRED", "POWERSHELL_NOT_FOUND", "REMOTE_PROFILE_INVALID", "REMOTE_PROFILE_NOT_FOUND", "REMOTE_PROFILE_UNREADABLE", "REMOTE_SKILL_NOT_FOUND", "REMOTE_TARGET_ACTIVE", "REMOTE_TARGET_NOT_CONFIGURED", "RUNTIME_DIR_UNWRITABLE", "SANDBOX_UNAVAILABLE", "SESSION_CLOSED", "SESSION_LIMIT_REACHED", "SESSION_NOT_FOUND", "SYMLINK_ESCAPE", "TARGET_NOT_SELECTED", "TTY_UNSUPPORTED", "UNSUPPORTED_ENCODING"]
```
...
The default catalog has 36 tools, including `view_image`. Setting
...
Inputs: `"path"`, `"content"`, `"encoding"`, `"dry_run"`, `"timeout_ms"`.

Annotations: `{"title":"Write file","readOnlyHint":false,"destructiveHint":true,"idempotentHint":false,"openWorldHint":false}`.

Writes UTF-8 text to the active target. In local mode it uses the same atomic
commit path as patches; in remote mode it routes to `remote_write_file`.
...
Annotations: `{"title":"List remote profiles","readOnlyHint":true,"destructiveHint":false,"idempotentHint":true,"openWorldHint":false}`.

Lists locally saved remote profile names and metadata roots. It never returns
passwords, private keys, or encrypted credential XML.

### remote_exec

Inputs: `"profile"`, `"command"`, `"remote_dir"`, `"timeout_ms"`, `"max_output_bytes"`, `"pty"`, `"remote_root"`.

Annotations: `{"title":"Remote exec","readOnlyHint":false,"destructiveHint":true,"idempotentHint":false,"openWorldHint":true}`.

Runs a bounded command through the selected local remote profile. Credentials are
loaded only from local profile storage.

### remote_list_dir

Inputs: `"profile"`, `"path"`, `"remote_dir"`, `"max_entries"`, `"timeout_ms"`, `"remote_root"`.

Annotations: `{"title":"Remote list directory","readOnlyHint":true,"destructiveHint":false,"idempotentHint":true,"openWorldHint":true}`.

### remote_read_file

Inputs: `"profile"`, `"path"`, `"remote_dir"`, `"start_line"`, `"end_line"`, `"max_bytes"`, `"timeout_ms"`, `"remote_root"`.

Annotations: `{"title":"Remote read file","readOnlyHint":true,"destructiveHint":false,"idempotentHint":true,"openWorldHint":true}`.

### remote_list_files

Inputs: `"profile"`, `"path"`, `"remote_dir"`, `"patterns"`, `"glob"`, `"exclude_patterns"`, `"include_hidden"`, `"include_ignored"`, `"max_results"`, `"sort"`, `"timeout_ms"`, `"remote_root"`.

Annotations: `{"title":"Remote list files","readOnlyHint":true,"destructiveHint":false,"idempotentHint":true,"openWorldHint":true}`.

Lists remote files using server-side `find` and local result filtering.

### remote_search_text

Inputs: `"profile"`, `"query"`, `"path"`, `"remote_dir"`, `"regex"`, `"case_sensitive"`, `"include_globs"`, `"glob"`, `"exclude_globs"`, `"context_lines"`, `"max_results"`, `"max_preview_bytes"`, `"timeout_ms"`, `"remote_root"`.

Annotations: `{"title":"Remote search text","readOnlyHint":true,"destructiveHint":false,"idempotentHint":true,"openWorldHint":true}`.

Searches remote files with `rg` when available and falls back to `grep`.

### remote_write_file

Inputs: `"profile"`, `"path"`, `"content"`, `"remote_dir"`, `"timeout_ms"`, `"remote_root"`.

Annotations: `{"title":"Remote write file","readOnlyHint":false,"destructiveHint":true,"idempotentHint":false,"openWorldHint":true}`.

### remote_apply_patch

Inputs: `"profile"`, `"patch"`, `"dry_run"`, `"remote_dir"`, `"timeout_ms"`, `"remote_root"`.

Annotations: `{"title":"Remote apply patch","readOnlyHint":false,"destructiveHint":true,"idempotentHint":false,"openWorldHint":true}`.

Applies add/update/delete/move envelopes to remote UTF-8 text files. Remote
multi-file patches are not transactionally rolled back across SSH operations.

### remote_git_status

Inputs: `"profile"`, `"path"`, `"include_untracked"`, `"max_entries"`, `"remote_dir"`, `"timeout_ms"`, `"remote_root"`.

Annotations: `{"title":"Remote git status","readOnlyHint":true,"destructiveHint":false,"idempotentHint":true,"openWorldHint":true}`.

### remote_git_diff

Inputs: `"profile"`, `"path"`, `"paths"`, `"staged"`, `"unstaged"`, `"context_lines"`, `"max_bytes"`, `"remote_dir"`, `"timeout_ms"`, `"remote_root"`.

Annotations: `{"title":"Remote git diff","readOnlyHint":true,"destructiveHint":false,"idempotentHint":true,"openWorldHint":true}`.

### remote_git_log

Inputs: `"profile"`, `"path"`, `"ref"`, `"max_count"`, `"skip"`, `"remote_dir"`, `"timeout_ms"`, `"remote_root"`.

Annotations: `{"title":"Remote git log","readOnlyHint":true,"destructiveHint":false,"idempotentHint":true,"openWorldHint":true}`.

### remote_git_show

Inputs: `"profile"`, `"rev"`, `"path"`, `"paths"`, `"include_diff"`, `"context_lines"`, `"max_bytes"`, `"remote_dir"`, `"timeout_ms"`, `"remote_root"`.

Annotations: `{"title":"Remote git show","readOnlyHint":true,"destructiveHint":false,"idempotentHint":true,"openWorldHint":true}`.

### remote_git_blame

Inputs: `"profile"`, `"path"`, `"rev"`, `"start_line"`, `"end_line"`, `"max_lines"`, `"remote_dir"`, `"timeout_ms"`, `"remote_root"`.

Annotations: `{"title":"Remote git blame","readOnlyHint":true,"destructiveHint":false,"idempotentHint":true,"openWorldHint":true}`.

### remote_upload

Inputs: `"profile"`, `"local_path"`, `"remote_path"`, `"remote_dir"`, `"timeout_ms"`, `"remote_root"`.

Annotations: `{"title":"Remote upload","readOnlyHint":false,"destructiveHint":true,"idempotentHint":false,"openWorldHint":true}`.

### remote_download

Inputs: `"profile"`, `"remote_path"`, `"local_path"`, `"remote_dir"`, `"timeout_ms"`, `"remote_root"`.

Annotations: `{"title":"Remote download","readOnlyHint":false,"destructiveHint":true,"idempotentHint":false,"openWorldHint":true}`.
